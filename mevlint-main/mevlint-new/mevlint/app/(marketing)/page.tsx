...
export default function Home() {
  ...
  return (
    <>
    <div ref={mainRef} className="bg-[#050505] text-[#e5e5e5] font-sans selection:bg-[#83E483] selection:text-black overflow-x-hidden tracking-tight">
      ...
      {/*<Hero />*/}
          ...
          <div className="space-y-4">
            {ALL_PROFESSIONS.map((role) => (
              <div key={role.id} className={`border-b border-black/10 transition-all duration-500 overflow-hidden ${activeRole === role.id ? 'bg-black/5' : ''}`}>
                ...
                <div className={`transition-all duration-700 ease-in-out ${activeRole === role.id ? 'max-h-125 opacity-100 py-12 px-16' : 'max-h-0 opacity-0'}`}>
        ...
        <section className="reveal-up py-40 bg-[#f5f5f5] text-black">
          <div className="px-8 md:px-20 lg:px-40 flex flex-col md:flex-row justify-between items-center gap-12">
             <div className="flex-1">
               <div className="flex items-center gap-4 mb-8">
                 <Cross size={24} strokeWidth={3} className="text-[#ff4d4d] animate-pulse transition duration-100" />
                 <span className="text-[10px] sm:text-[10px] md:text-[12px] lg:text-[12px] xl:text-[12px]uppercase tracking-[1em] sm:tracking-[0.5em] font-black opacity-30 uppercase">Vision // 08</span>
               </div>
               <h3 className="text-7xl md:text-9xl font-san font-black uppercase italic tracking-tighter leading-none mb-10">Flex <br/> Skills</h3>
               <p className="sm:text-sm md:text-[18px] font-normal opacity-60 max-w-sm">
                Don&#39;t let <s>brain-rot algorithms</s> bury your work <br />
                find your <b>niche audience</b>, showcase your <b>skills</b>, and <b>collaborate</b> with creators who actually get your <b>vision</b>.
               </p>
             </div>

             <div className="flex-1 grid grid-cols-2 gap-4">
                <div className="aspect-square bg-black h-full p-6 lg:p-10 flex flex-col justify-between hover:scale-[1.02] transition-transform cursor-crosshair">
                   <ResponsiveIcon 
                      icon={Clapperboard} 
                      className="text-[#ffcc33] transition-transform duration-700" 
                    />
                   <p className="text-[10px] font-black uppercase tracking-widest text-white">Post <br/> Collaborate</p>
                </div>
                <div className="aspect-square border-4 border-black p-10 flex flex-col justify-between h-auto gap-2 sm:gap-2 md:gap-4 lg:g-4 xl:g-4transition-transform hover:transition-all group">
                  <div className="flex flex-col gap-2 md:gap-4">
                  <div className="h-1 w-full bg-black mb-4 group-hover:w-2/3 group-hover:bg-[#ff4d4d] transition-all"></div>
                  <div className="h-1 w-2/3 bg-[#ff4d4d] mb-4 group-hover:w-full group-hover:bg-black transition-all"></div>
                  <div className="hidden sm:block h-1 w-full bg-black mb-4 group-hover:w-2/3 group-hover:bg-[#ff4d4d] transition-all"></div>
                  <div className="hidden sm:block h-1 w-2/3 bg-[#ff4d4d] mb-4 group-hover:w-full group-hover:bg-black transition-all"></div>
                  </div>
                   <p className="text-[10px] font-black uppercase tracking-widest text-black">Critics <br/> Popularity</p>
                </div>
                <div className="col-span-2 p-6 lg:p-12 border-2 border-black flex items-center justify-between group cursor-pointer">
                   <span className="text-lg sm:text-lg md:text-2xl lg:text-2xl xl:text-4xl font-black uppercase italic tracking-tighter">Genuine Creators</span>
                   <ResponsiveIcon 
                      icon={Sun}
        ...
        {/* 9. ENDING SEQUENCE */}
        <section className="reveal-up min-h-screen flex flex-col justify-center items-center text-center px-8 py-6 relative">
            <Sparkles className="text-[#83E483] mb-12 opacity-70 animate-pulse duration-200" size={60} />
  
            <div className="relative space-y-2 transform-gpu">
                <h3 className="text-[6vw] sm:text-[6vw] md:text-[4vw] lg:text-[4vw] xl:text-[4vw] text-[#ffffff] italic leading-none cursor-pointer">
                    If you&lsquo;re <b>building worlds</b>
                </h3>
                <h2 className="text-[10vw] sm:text-[10vw] md:text-[10vw] lg:text-[10vw] xl:text-[8vw]  font-black italic tracking-tighter uppercase leading-none text-[#83E483] cursor-pointer">
                    you belong here.
                </h2>
            </div>
  
            <div className="mt-12 sm:mt-14 md:mt-16 lg:mt-18 xl:mt-20 w-fit h-fit flex flex-col items-center gap-8 justify-center pt-4 bg-[#ffffff] transform-gpu rounded-xl">
                <a href="/" className="group w-full h-full flex px-8 items-center gap-4 text-sm text-black uppercase tracking-widest border-b rounded-xl hover:border-b-8 border-white/60 pb-4 hover:border-[#83E483] transition-all duration-500 hover:tracking-normal font-black font-mono">
                    Join Us <ArrowRight className="group-hover:translate-x-4 transition-transform duration-300 text-[#83E483]" />
                </a>
            </div>
        </section>
      </main>
